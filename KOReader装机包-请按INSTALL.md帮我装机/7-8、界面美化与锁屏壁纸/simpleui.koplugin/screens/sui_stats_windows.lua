...
local function _fmtDateRange(start_ts, finish_str)
    ...
    local finish_part = string.format("%d.%d.%d", sf.y % 100, sf.m, sf.d)
    if not ss then return finish_part end
    local start_part
    if ss.y == sf.y then
        start_part = string.format("%02d.%02d.%02d", ss.y % 100, ss.m, ss.d)
        finish_part = string.format("%02d.%02d", sf.m, sf.d)
    else
        start_part = string.format("%02d.%02d.%02d", ss.y % 100, ss.m, ss.d)
        finish_part = string.format("%02d.%02d.%02d", sf.y % 100, sf.m, sf.d)
    end

    return start_part .. "-" .. finish_part
end
...
local function _riFmtDate(ts)
    ...
    return string.format("%02d.%02d.%02d", t.year % 100, t.month, t.day)
end
