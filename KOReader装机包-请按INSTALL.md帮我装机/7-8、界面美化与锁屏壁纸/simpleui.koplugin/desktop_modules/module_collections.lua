...
-- getDims(scale, thumb_scale, inner_w)
...
-- inner_w:     available width for the 5-slot row (w - PAD*2). Used to compute
--              the auto-fit cover-width baseline (see below). When nil (a call
--              site without the real width yet), falls back to the small fixed
--              _BASE_COLL_W constant so this still returns something reasonable.
local function getDims(scale, thumb_scale, inner_w)
    scale       = scale       or 1.0
    ...
    -- Cover width: base is always the "auto-fit" size — the 5 fixed slots
    -- (stack_cell_w each) + 4 PAD gaps fill inner_w exactly. Scale/Cover Size
    -- (cs) is applied as a multiplier ON TOP of that baseline, never as a
    -- switch to an unrelated fixed constant. This mirrors the fix applied to
    -- the book-row modules (sui_book_row.lua): 100% always fills the row,
    -- and any other percentage grows/shrinks continuously from what's
    -- already visible, instead of jumping to a smaller/larger unrelated
    -- base the moment the user moves off 100%.
    local coll_w
    if inner_w then
        local autofit_coll_w = math.max(1, math.floor((inner_w - 4 * PAD) / 5) - stack_extra)
        coll_w = (cs == 1.0) and autofit_coll_w or math.max(1, math.floor(autofit_coll_w * cs))
    else
        coll_w = math.max(1, math.floor(_BASE_COLL_W * cs))
    end
    local coll_h = math.max(1, math.floor(coll_w * (_BASE_COLL_H / _BASE_COLL_W)))
...
function M.build(w, ctx)
    ...
    local d           = getDims(scale, thumb_scale, inner_w)
...
function M.getHeight(ctx)
    ...
    -- coll_h (and therefore coll_cell_h) is derived from the auto-fit
    -- coll_w, so getHeight() needs the same inner_w build() uses — see the
    -- note on getDims() above. Falls back to a screen-width estimate when
    -- ctx doesn't carry the real column width yet (e.g. early layout pass).
