...
-- Raw key under which the pre-plugin native screensaver type is remembered,
-- so KOReader's own screensaver can be restored when the plugin bows out
-- (e.g. when suspending outside of a book with "show in file manager" off).
local NATIVE_SS_TYPE_KEY = "customisable_ss_native_type"

local function getNativeScreensaverType()
    local t = css_settings.plugin():readSetting(NATIVE_SS_TYPE_KEY)
    if t == nil or t == "" or t == "customisable_ss" then
        return "cover" -- default: cover of the currently read book
    end
    return t
end
    ...
    menu_items.customisable_sleep_screen = {
        ...
        sub_item_table_func = function()
            ...
            local enable_item = {
                ...
                callback = function()
                    if G_reader_settings:readSetting(SETTINGS.TYPE) == "customisable_ss" then
                        -- Disabling: restore the remembered native screensaver type
                        G_reader_settings:saveSetting(SETTINGS.TYPE, getNativeScreensaverType())
                    else
                        -- Enabling: remember the current native type before taking over
                        css_settings.plugin():saveSetting(
                            NATIVE_SS_TYPE_KEY,
                            G_reader_settings:readSetting(SETTINGS.TYPE)
                        )
                        css_settings.flush()
...
function CustomisableSleepScreen:_installScreensaverHook()
    ...
    -- Fall back to KOReader's own screensaver (e.g. current book cover) when
    -- the plugin does not want to draw (outside of a book). This temporarily
    -- restores the native screensaver_type, re-runs the native setup() so it
    -- re-reads its settings for the native mode, shows it, then puts our own
    -- type back so the in-book path keeps working.
    local function runNativeScreensaver(ss_self)
        local saved_type = G_reader_settings:readSetting(SETTINGS.TYPE)
        G_reader_settings:saveSetting(SETTINGS.TYPE, getNativeScreensaverType())
        pcall(function() ss_self:setup() end)
        local ok, err = pcall(function()
            css._screensaver_hook:raw_call(ss_self)
        end)
        G_reader_settings:saveSetting(SETTINGS.TYPE, saved_type)
        if not ok then
            logger.warn("[Customisable Sleep Screen] native screensaver fallback failed: " .. tostring(err))
        end
    end
    ...
    self._screensaver_hook = util.wrapMethod(Screensaver, "show", function(ss_self)
        ...
        if ui and ui.document then
            ...
            local show_in_fm = require("css_infobox_render").getSetting("SHOW_IN_FILEMANAGER")
            if not show_in_fm then
                -- Outside of a book with "show in file manager" off:
                -- let KOReader's own screensaver handle it (e.g. book cover).
                return runNativeScreensaver(ss_self)
            end
            local book_data = ib.loadLastBookData()
            if book_data then
                widget = ib.buildInfoBox(nil, nil, book_data)
        ...
        if not widget then return runNativeScreensaver(ss_self) end
