...
function FilebrowserPlus:resetPassword()
    ...
    if status == 0 then
        local info = InfoMessage:new{
            timeout = 15,
            text = string.format(
                "用户 %s 的密码已设置为 %s\n可通过 filebrowser 网页面板修改！", username,
                newPass)
        }
        UIManager:show(info)
    else
...
function FilebrowserPlus:start()
    ...
    if not util.pathExists(self.filebrowserplus_dataPath) then
        ...
        if util.pathExists(self.filebrowserplus_dataPath) then
            ...
            UIManager:show(InfoMessage:new{
                icon = "notice-warning",
                text = _("创建数据目录失败。\n" .. "请检查路径或权限。\n\n" ..
                             "注意：Kindle 上只有 /mnt/us 下的位置可写入；\n" ..
                             "Kobo 上只有 /mnt/onboard 下的位置可写入。")
            })
            return
        end
    end
    ...
    if status == 0 then
        ...
        if self.auto_stop_minutes and self.auto_stop_minutes > 0 then
            auto_stop_msg = "\n" .. T(_("%1 分钟后自动停止"), self.auto_stop_minutes)
        end

        local info = InfoMessage:new{
            timeout = timeout_duration,
            text = _("FilebrowserPlus 服务器已启动。") .. auto_stop_msg .. extra_info
        }
        UIManager:show(info)
        ...
        local info = InfoMessage:new{
            icon = "notice-warning",
            timeout = 15,
            text = _("FilebrowserPlus 服务器启动失败")
        }
        UIManager:show(info)
    end
...
function FilebrowserPlus:checkAutoStop()
    ...
    if os.time() >= self.stop_deadline then
        ...
        self:stop(true)
        UIManager:show(InfoMessage:new{
            text = _("FilebrowserPlus 服务器已超时自动停止"),
            timeout = 3
        })
    else
...
function FilebrowserPlus:stop(is_auto)
    ...
    if status == 0 then
        ...
        UIManager:show(InfoMessage:new{
            icon = "notice-warning",
            timeout = 15,
            text = _("FilebrowserPlus 服务器停止失败")
        })
    end
...
function FilebrowserPlus:show_dataPath_dialog(touchmenu_instance)
    self.dataPath_dialog = InputDialog:new{
        title = _("输入 FilebrowserPlus 数据路径"),
...
function FilebrowserPlus:addToMainMenu(menu_items)
    local sub_item_table = {{
        text_func = function()
            return T(_("FilebrowserPlus 端口（%1）"), self.filebrowserplus_port)
        end,
        keep_menu_open = true,
        ...
        text_func = function()
            return T(_("FilebrowserPlus 数据路径（%1）"), self.filebrowserplus_dataPath)
        end,
        keep_menu_open = true,
        enabled_func = function()
            return not self:isRunning()
        ...
        text_func = function()
            if self.auto_stop_minutes and self.auto_stop_minutes > 0 then
                return T(_("自动停止超时（%1 分钟）"), self.auto_stop_minutes)
            else
                return _("自动停止超时（已关闭）")
            end
        end,
    ...
    menu_items.filebrowserplus = {
        text_func = function()
            if self:isRunning() then
                ...
                return _("FilebrowserPlus（长按打开设置）")
            end
        end,
