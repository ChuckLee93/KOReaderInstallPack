--[ reading insights popup v1.0.40 ]  -- fixed: translate all phrases
-- ABOUT:
...
local PATCH_L10N = {
    ...
    zh = {
        ...
        ["pages"] = "页",
        ["TODAY"] = "今天",
        ["No weekly streak"] = "无连续周",
        ["No daily streak"] = "无连续天",
        ["CURRENT STREAK"] = "当前连续",
        ["BEST STREAK"] = "最长连续",
        ["DAYS READ PER MONTH"] = "每月阅读天数",
        ["HOURS READ PER MONTH"] = "每月阅读小时",
        ["Reading statistics: reading insights"] = "阅读统计：阅读洞察",
        ["Unknown"] = "未知",
        ["No books read"] = "未读书籍",
        ["No books read in %1"] = "%1 未读书籍",
        ["No books read in "] = "未读书籍于 ",
        ["%1 - Book Read (%2)"] = "%1 - 读过书 (%2)",
        ["%1 - Books Read (%2)"] = "%1 - 读过书籍 (%2)",
    },
}
...
local function clearCacheIfRequired()
    -- checks and calls clearCache() as per req.
    ...
    if refreshOnlyOncePerDay and (cache_timestamps.lastRefreshed > ts_midnight_today) then
        return
    end

    if (cache_timestamps.statsSynced > cache_timestamps.fullClear) then
        --if stats db was modified via sync
    ...
    if (latest_db_mod_timestamp > cache_timestamps.partialClear) then
        --if stats db was modified locally
    ...
    if latest_db_mod_timestamp < (ts_midnight_today - 86400) and 	--if stats db was last modified more than two midnights ago
...
local function buildYearHeader(popup_self, font_section, layout, yearRange)
    ...
    tappable_year_label.ges_events = {
        Tap = {
            GestureRange:new{ ges = "tap", range = function() return tappable_year_label.dimen end, }
        },
        Hold = {
            GestureRange:new{ ges = "hold", range = function() return tappable_year_label.dimen end, }
        },
    }
    ...
    local prev_widget = prev_enabled and navButton(tostring(selected_year - 1), selected_year - 1, 0) or HorizontalSpan:new{ width = nav_width }
    local next_widget = next_enabled and navButton(tostring(selected_year + 1), selected_year + 1, 1) or HorizontalSpan:new{ width = nav_width }
...
local function buildYearlyRow(popup_self, yearly_stats, fonts, layout)
    ...
    local left_line = buildValueLine( fonts.value, fonts.streakLabel, layout.column_gap, left_value, left_unit )
    local left_line_dimen = left_line:getSize()

    local pages_val = buildValueLine( fonts.value, fonts.streakLabel, layout.column_gap, formatCount(yearly_stats.pages), N_("page read", "pages read", yearly_stats.pages) )
    ...
    left_cell.ges_events = {
        Tap = {
            GestureRange:new{ ges = "tap", range = function() return left_cell.dimen end, }
        },
    }
    ...
    right_cell.ges_events = {
        Tap = {
            GestureRange:new{ ges = "tap", range = function() return right_cell.dimen end, }
        },
    }
...
local function buildMonthlyChart(popup_self, monthly_data, layout, fonts)
    ...
    local function createBarRow(data_slice)
        ...
        for i, m in ipairs(data_slice) do
            ...
            tappable_bar.ges_events = {
                Tap = {
                    GestureRange:new{ ges = "tap", range = function() return tappable_bar.dimen end, }
                },
            }
    ...
    --FocusManager
    if #foc_mgr_thirdRow > 0 then
        table.insert(popup_self.layout, foc_mgr_thirdRow)
    end
    if #foc_mgr_fourthRow > 0 then
        table.insert(popup_self.layout, foc_mgr_fourthRow)
    end
    return chart
end
...
function ReadingInsightsPopup:onShow()
    UIManager:setDirty(self, function() return "ui", self.popup_frame.dimen end)
    return true
end
