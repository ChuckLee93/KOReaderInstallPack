    ...
    local function recurse(item_table, path, text, icon, depth)
        ...
        for i, v in ipairs(item_table) do
            if type(v) == "table" and not v.ignored_by_menu_search then
                ...
                -- [MenuSearchFix 2026-08-29] 自定义补丁/插件可能注册无 text 的条目，
                -- 原逻辑拼接 walk_text 时遇 nil 会让 luajit 崩溃、KOReader 整体退出
                -- (2026-08-29 23:12 搜索菜单时实际触发)。此处改为占位符并记录位置。
                if type(entry_text) ~= "string" then
                    local _msf_logger = require("logger")
                    _msf_logger.warn("[MenuSearchFix] menu entry without text at", path .. "." .. i)
                    entry_text = "\u{2026}"
                end
