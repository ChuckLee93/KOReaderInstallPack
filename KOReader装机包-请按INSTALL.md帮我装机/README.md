...
| 诊断探针 | 三枚性能探针：重载溯源（谁触发了界面重建）、翻页计时（哪个主循环阶段 ≥300ms）、任务计时（哪个插件的任务卡住界面），全部写入 `crash.log`，平时零开销。日后出问题 AI 可远程读日志排障 | 必装（静默） |
...
或 USB 插着电脑，都可以把问题交给 AI 助手——它会读取探针日志
`koreader/crash.log`（搜 `[ReloadDiag]`/`[PageTurnPerf]`/`[TaskPerf]` 标记）远程诊断。装机 AI 会在收尾时
向用户说明这一点（INSTALL.md §8.5-E6）。
