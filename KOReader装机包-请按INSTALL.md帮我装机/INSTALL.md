...
│   ├── patches/                  8 个功能补丁 + 3 个诊断探针补丁（重载溯源 + 翻页计时 + 任务计时）
...
├── 备用组件/                     ← 备用：封面增强（KOLBooklet、cover-setter，默认不安装）
│                                   + 个人设置同步/（作者实测配置，装机收尾 E1 询问是否同步，见 §8.5-E1）
└── 文档/
...
> · **内置诊断**：装了三枚"性能探针"——平时零开销，翻页卡顿、界面冻结、
>    重载变慢时自动在 crash.log 记下耗时和元凶（精确到哪个插件哪行代码），
>    找人帮忙时发个文件就能远程排查；
...
3. 覆盖完成后回到 §2 从第 2 步走**全量装机**（含 frontend 覆盖、插件修复替换、
   三探针、手势；此路径下 §7 精简默认执行，因为底座已换成本包基线）。
...
| 5 | 诊断探针（重载溯源 + 翻页计时 + 任务计时） | 必装 | C2（其中 3 个补丁） |
...
4. 阅读小票（4 号）、诊断探针（5 号）是 C2 里的独立补丁，可单独跳过。
   **5 号探针建议尽量保留**（售后远程排查全靠它）；用户明确拒绝则不装，
   并在 E6 售后说明中把远程通道一段改为"仅 USB"。
5. **部分安装时 §7（删除 26 个官方插件）默认不执行**——对方设备可能已有自己的
   插件布局；仅当用户明确说"帮我精简/删掉多余官方插件"时才执行。
6. 翻页动画（11 号）仍过机型门（仅新机型）；低内存保护（C7）不属编号组件，
   仍按机型单独询问（用户不想被问就跳过）。
7. 收尾五项（§8.5）只问与所装组件相关的：E1（作者同款设置同步）任何装机都问；
   E2 仅当装了 1 号；E3 仅当装了 10 号；E4 只要装了底座 FilebrowserPlus 就问；E5 照旧。
...
以及三个诊断探针（ChuckLee93 原创）：
- `2-reload-diag.lua`（重载溯源探针）：同一本书发生 ReaderUI 重建（reloadDocument /
  switchDocument，每次冻结界面约 5-7 秒）的瞬间，把发起方完整调用栈写进 `crash.log`
  （搜 `[ReloadDiag]` 定位元凶）。
- `2-pageturn-perf.lua`（翻页计时探针）：给 UI 主循环各阶段计时（输入分发 / 定时任务 /
  重绘 / 显式 GC），任一阶段 ≥300ms（GC ≥200ms）就在 `crash.log` 记录耗时与调用栈
  （搜 `[PageTurnPerf]`）。翻页卡顿在 crash.log 里本来是隐形的，靠它显形。
- `2-task-perf.lua`（任务计时探针）：给每条定时任务单独计时，任一任务 ≥300ms 就在
  `crash.log` 记下模块名和行号（搜 `[TaskPerf]`），直接点名是哪个插件的任务卡住了界面。
三者互补：一个抓"谁触发了重建"，一个抓"哪个主循环阶段慢"，一个抓"哪个任务慢"。
三个探针均只读不写配置、不含账号密钥、平时零开销（不打点不计时的情况下无任何额外
代码路径），随核心必装，无需询问。
...
2. 启动后引导用户逐项验证（设备端操作，WorkBuddy 口述）：
   ...
   - 可选：若电脑与 Kindle 在同一 WiFi，WorkBuddy 可经 §3 远程通道读
     `crash.log` 首屏（确认三枚探针补丁加载无报错）核对装机结果（此时提醒用户别让 Kindle 睡着）；
     不方便连 WiFi 就纯靠上面的口述清单核对。
3. 验证全部通过后，进入 **§8.5 装机后询问（收尾五项 + 售后说明）**逐项询问用户；
   更细致的个性化设置清单见 `文档/manual-steps.md` B 节。

---

## 8.5 装机后询问（收尾五项 + 售后说明，§8 验证通过后逐项进行）

装机验证完成后，WorkBuddy **主动**向用户提出以下五项收尾询问。逐项问，
每项处理完再进入下一项，不要一口气全部抛出。五项问完，最后按 E6 向用户
做售后说明（必做），之后装机流程才告结束。

### E1. 作者同款设置同步（收尾第一问，任何装机都问）

1. `【询问用户】`："要不要把作者调好的全套设置直接搬过来？——排版、页眉、
   菜单、手势、书库卡片、锁屏壁纸、查词、AI 伴读提示词这些实测配置一次到位，
   不用你自己一项项调。"
   （这套设置在包的 `备用组件/个人设置同步/` 里，从作者实机导出，**已剔除
   所有账号、密码、API key 和个人阅读数据**，只有纯配置。）
2. 用户同意 → 执行（KOReader 需已退出）：
   1. 先备份：把设备上即将被覆盖的文件（`/mnt/us/koreader/settings.reader.lua`
      及 `settings/` 下的同名文件）复制到装机备份目录 `backup/`；
   2. 复制 `备用组件/个人设置同步/settings.reader.lua` →
      `/mnt/us/koreader/settings.reader.lua`；
   3. 将 `备用组件/个人设置同步/settings/` 下的全部文件合并复制到
      `/mnt/us/koreader/settings/`（同名覆盖）；
   4. 完成后重启 KOReader 生效，告诉用户"打开看看，不满意随时可以恢复"
      （恢复 = 从 backup/ 拷回）。
3. 用户拒绝 → 跳过；preset 键块已覆盖大部分常用设置，对方自行微调即可。
4. **顺序要求：本项必须在 E2（Z-Library 登录）、E3（AI 伴读 API key）之前
   完成**——设置整文件覆盖会清空这两类凭据；若中途补做本项，覆盖后需重新
   登录/填 key 一遍。
5. 下载目录说明（用户问起时告知）：设置里 Z-Library 的下载目录是
   `/mnt/us/BOOKS`（Kindle 磁盘根目录的 BOOKS 文件夹）。`/mnt/us` 是所有
   Kindle 通用的用户存储挂载点，路径任何机型都有效；文件夹不存在时插件
   下载前会自动创建，无需手动建。
...
### E3. AI 伴读的 API key（仅限装了 KOAI 的用户）
...
### E4. 无线传书网址（装了 FilebrowserPlus 的用户——即所有装机用户）
...
### E6. 售后说明（五项问完后的最后一段，必做）

装机已包含**三个诊断探针**（随核心静默安装）：
- **重载溯源探针**（补丁 `2-reload-diag.lua`）：同一本书发生界面重建（每次冻结约
  5-7 秒）的瞬间，自动把发起方的完整调用栈写进 `crash.log`（搜 `[ReloadDiag]`
  即可定位是谁发起的重建）。
- **翻页计时探针**（补丁 `2-pageturn-perf.lua`）：给 UI 主循环各阶段计时，任一阶段
  ≥300ms 就把耗时和调用栈写进 `crash.log`（搜 `[PageTurnPerf]`）。
- **任务计时探针**（补丁 `2-task-perf.lua`）：给每条定时任务单独计时，任一任务
  ≥300ms 就把模块名和行号写进 `crash.log`（搜 `[TaskPerf]`），直接点名卡界面的插件。
三个探针都只读不写配置、不含账号密钥、平时零开销，日志只存在对方自己设备里。

向用户念这段话（可换说法，意思到位即可）：

> 装机时给你装了三个"诊断探针"：平时完全无感、零开销；一旦出现翻页卡顿、
> 界面冻结、莫名变慢，它们会自动在日志里记下耗时和"元凶"（精确到哪个插件）。
...
>    读取诊断日志，边看边修；
> ② 或者 USB 插上电脑——把 `koreader/crash.log` 拷出来发给 AI 也可以。
...
| 现象 | 处理 |
|---|---|
| **任何用户报障（装机后求助）** | 先读探针日志 `crash.log`（远程通道 `GET /api/raw/mnt/us/koreader/crash.log`，或 USB 插电脑直接读 `K:/koreader/crash.log`；搜 `[ReloadDiag]`/`[PageTurnPerf]`/`[TaskPerf]` 标记：分别定位"谁触发了界面重建"、"哪个主循环阶段慢"、"哪个定时任务卡"），再按线索定位相关插件/补丁文件；日志无标记时按现象描述排查。用户求助的接入方式见 §8.5-E6 |
