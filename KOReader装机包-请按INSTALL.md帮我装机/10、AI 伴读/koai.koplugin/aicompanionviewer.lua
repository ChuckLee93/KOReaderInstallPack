...
-- 合并版说明：
-- 本文件以 AI Reading Assistant 的查看器为基座（保留追问、保存笔记、滚动、update）。
-- 新增 rich_text 字段：KOAI 精读结果显示时为 true，启用富文本分层与更大字号；
-- 默认（轻量模式）保持原行为：剥离 Markdown 符号、x_smallinfofont、两端对齐。
-- richtext_formatter 采用懒加载，轻量模式下不会占用内存。
...
local AICompanionViewer = FocusManager:extend {
  ...
  text_face = nil,                -- computed in init
...
function AICompanionViewer:init()
  ...
  -- KOAI 精读模式：更大字号、不对齐、富文本分层；轻量模式保持原版行为。
  if self.rich_text then
    local ok_cfg, config = pcall(function() return require("configuration") end)
    local font_size = (ok_cfg and config and tonumber(config.result_font_size)) or 24
    self.text_face = Font:getFace("smallinfofont", font_size)
    self.justified = false
  else
    self.text_face = self.text_face or Font:getFace("x_smallinfofont")
    self.justified = true
  end

  if self.text then
    if self.rich_text then
      -- v1.1：所有旧版与新版内容在显示时自动套用层级样式。
      -- 原始本地卡片、复盘和已保存笔记不被改写，也不会重新调用 AI。
      local ok_fmt, RichTextFormatter = pcall(function() return require("richtext_formatter") end)
      if ok_fmt and RichTextFormatter and RichTextFormatter.format then
        self.text = RichTextFormatter.format(self.text)
      end
    else
  ...
  self.movable = MovableContainer:new {
    ...
    ignore_events = {
      ...
      -- but may happen while we are selecting text: we need to check
...
function AICompanionViewer:update(new_text, new_response)
  ...
  local updated_viewer = AICompanionViewer:new {
    ...
    rich_text = self.rich_text,  -- Preserve KOAI rich text mode
  }
